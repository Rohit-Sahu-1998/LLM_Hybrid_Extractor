...
def load_extractor():
    """
    Load hybrid extractor once.
    Clear Streamlit cache if you update backend code and do not see changes.
    """
...
def save_uploaded_file(uploaded_file):
    """
    Save uploaded file temporarily so backend can process it.
    """
...
def draw_highlight_on_image(
    ...
    """
    Draw highlight polygon using OCR bbox.

    Important:
    - Qwen does not provide bounding boxes.
    - The box drawn here is the matched OCR region for the Qwen value.
    """
...
def flatten_result_for_actions(result: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    Flatten nested result into row-wise records used by the right-side action panel.
    """
    rows = []
...
st.markdown(
    '<div class="sub-title">Left side shows the document page. Right side shows extracted fields. Qwen confidence, Qwen logprob, and PaddleOCR confidence are displayed separately. Highlight box is drawn from the matched OCR bbox for the Qwen value.</div>',
    unsafe_allow_html=True
)
    ...
    if st.session_state["result"] is not None and st.session_state["images"] is not None:
        ...
        with right_col:
            ...
            if not page_rows:
                ...
                header_cols = st.columns([1.6, 2.3, 1.0, 1.0, 1.0, 1.1])
                header_cols[0].markdown("**Field**")
                header_cols[1].markdown("**Value / OCR Match**")
                header_cols[2].markdown("**Qwen Conf**")
                header_cols[3].markdown("**Logprob**")
                header_cols[4].markdown("**OCR Conf**")
                header_cols[5].markdown("**Action**")

                st.markdown("---")

                for idx, row in enumerate(page_rows):
                    c1, c2, c3, c4, c5, c6 = st.columns([1.6, 2.3, 1.0, 1.0, 1.0, 1.1])
